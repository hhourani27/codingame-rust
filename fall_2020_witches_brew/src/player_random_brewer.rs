...
pub fn play(
    ...
    while ctr_rcv.recv().unwrap() == true {
        ...
        let brew_moves: Vec<&game::Move> = valid_moves
            .slice()
            .iter()
            .filter(|m| matches!(*m, game::Move::BREW(o)))
            .collect();

        let chosen_move: &game::Move;

        if brew_moves.is_empty() == false {
            chosen_move = brew_moves.choose(&mut rand::thread_rng()).unwrap();
        } else {
            chosen_move = valid_moves.slice().choose(&mut rand::thread_rng()).unwrap();
        }
